...
func TestForm_Has(t *testing.T) {
	...
	has = form.Has("a")
	if !has {
		t.Error("shows form does not have field when it has")
	}
}
...
func TestForm_MinLength(t *testing.T) {
	...
	form.MinLength("x", 10) // a non-exsitent field
	if form.Valid() {
		t.Error("form shows min length for an non-existent field")
	}

	// use the invalid case above
...
func TestForm_IsEmail(t *testing.T) {
	...
	form.IsEmail("x") // a non-exsitent field
	if form.Valid() {
