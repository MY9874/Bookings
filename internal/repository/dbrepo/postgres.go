...
func (m *postgresDBRepo) InsertReservation(res models.Reservation) (int, error) {
	// if the reservation is not done in 3 sec, cancel it, it is likely that the user loses connection
	...
	err := m.DB.QueryRowContext(ctx, stmt,
		...
		time.Now(),
		time.Now(),
	).Scan(&newID) // scan the id it returns and store in newID
...
// SearchAvailabilityForAllRooms returns a slice of availiable rooms, if any for the given date range
...
func (m *postgresDBRepo) Authenticate(email, testPassword string) (int, string, error) {
	...
	var id int
	var hashedPassword string // password in the database
